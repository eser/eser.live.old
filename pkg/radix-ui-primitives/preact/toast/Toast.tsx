...
import * as DismissableLayer from "../dismissable-layer/mod.ts";
import { Portal } from "../portal/mod.ts";
...
const [ToastProviderProvider, useToastProviderContext] = createToastContext<
      ...
      if (viewport) {
        const handleKeyDown = (event: KeyboardEvent) => {
          ...
          if (isTabKey) {
            ...
            const index = sortedCandidates.findIndex((candidate) =>
...
type SwipeEvent =
  ...
  & Omit<
    CustomEvent<
      { originalEvent: React.PointerEvent; delta: { x: number; y: number } }
    >,
    "currentTarget"
  >;
...
const ToastImpl = React.forwardRef<ToastImplElement, ToastImplProps>(
  (props: ScopedProps<ToastImplProps>, forwardedRef) => {
    ...
    const swipeDeltaRef = React.useRef<{ x: number; y: number } | null>(
      null,
    );
                ...
                <Primitive.li
                  ...
                  onPointerMove={composeEventHandlers(
                    ...
                    (event) => {
                      ...
                      const clamp =
                        ["left", "up"].includes(context.swipeDirection)
                          ? Math.min
                          : Math.max;
                      const clampedX = isHorizontalSwipe ? clamp(0, x) : 0;
                      const clampedY = !isHorizontalSwipe ? clamp(0, y) : 0;
                      const moveStartBuffer = event.pointerType === "touch"
                        ? 10
                        : 2;
...
ToastImpl.propTypes = {
  type(props) {
    if (props.type && !["foreground", "background"].includes(props.type)) {
      const error =
        `Invalid prop \`type\` supplied to \`${TOAST_NAME}\`. Expected \`foreground | background\`.`;
      return new Error(error);
    }
    return null;
  },
...
interface ToastAnnounceProps
  extends
    Omit<Radix.ComponentPropsWithoutRef<"div">, "children">,
...
const ToastAnnounce: React.FC<ToastAnnounceProps> = (
  ...
  const context = useToastProviderContext(TOAST_NAME, __scopeToast);
  const [renderAnnounceText, setRenderAnnounceText] = React.useState(
    false,
  );
...
function getAnnounceTextContent(container: HTMLElement) {
  ...
  childNodes.forEach((node) => {
    ...
    if (isHTMLElement(node)) {
      const isHidden = node.ariaHidden || node.hidden ||
