...
const NavigationMenuProvider: React.FC<NavigationMenuProviderProps> = (
  ...
  const [viewport, setViewport] = React.useState<
    NavigationMenuViewportElement | null
  >(null);
  const [viewportContent, setViewportContent] = React.useState<
    Map<string, ContentData>
  >(new Map());
  const [indicatorTrack, setIndicatorTrack] = React.useState<
    HTMLDivElement | null
  >(null);
        ...
        <FocusGroupItem asChild>
          <Primitive.button
            ...
            onKeyDown={composeEventHandlers(props.onKeyDown, (event) => {
              const verticalEntryKey = context.dir === "rtl"
                ? "ArrowLeft"
                : "ArrowRight";
              const entryKey =
                { horizontal: "ArrowDown", vertical: verticalEntryKey }[
                  context.orientation
                ];
        ...
        <>
          <VisuallyHiddenPrimitive.Root
            ...
            onFocus={(event) => {
              ...
              const prevFocusedElement = event.relatedTarget as
                | HTMLElement
                | null;
...
>((props: ScopedProps<NavigationMenuIndicatorImplProps>, forwardedRef) => {
  ...
  const [activeTrigger, setActiveTrigger] = React.useState<
    NavigationMenuTriggerElement | null
  >(
    null,
  );
...
>((props: ScopedProps<NavigationMenuContentImplProps>, forwardedRef) => {
  ...
  const getItems = useCollection(__scopeNavigationMenu);
  const prevMotionAttributeRef = React.useRef<MotionAttribute | null>(
    null,
  );
      ...
      <DismissableLayer
        ...
        onPointerDownOutside={composeEventHandlers(
          ...
          (event) => {
            ...
            const isRootViewport = context.isRootMenu &&
...
interface NavigationMenuViewportProps
  extends
    Omit<NavigationMenuViewportImplProps, "children" | "activeContentValue"> {
  /**
   * Used to force mounting when more control is needed. Useful when
   * controlling animation with React animation libraries.
   */
...
>((props: ScopedProps<NavigationMenuViewportImplProps>, forwardedRef) => {
  ...
  const [content, setContent] = React.useState<
    NavigationMenuContentElement | null
  >(null);
    ...
    return (
      <FocusGroupCollection.ItemSlot scope={__scopeNavigationMenu}>
        <Primitive.button
          ...
          ref={forwardedRef}
          onKeyDown={composeEventHandlers(props.onKeyDown, (event) => {
            const isFocusNavigationKey = ["Home", "End", ...ARROW_KEYS]
              .includes(event.key);
            if (isFocusNavigationKey) {
              let candidateNodes = getItems().map((item) => item.ref.current!);
              const prevItemKey = context.dir === "rtl"
                ? "ArrowRight"
                : "ArrowLeft";
