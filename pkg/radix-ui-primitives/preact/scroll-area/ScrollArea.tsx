...
const [createScrollAreaContext, createScrollAreaScope] = createContextScope(
  SCROLL_AREA_NAME,
);
...
const [ScrollAreaProvider, useScrollAreaContext] = createScrollAreaContext<
...
const ScrollArea = React.forwardRef<ScrollAreaElement, ScrollAreaProps>(
  (props: ScopedProps<ScrollAreaProps>, forwardedRef) => {
    ...
    const [viewport, setViewport] = React.useState<
      ScrollAreaViewportElement | null
    >(null);
    const [content, setContent] = React.useState<HTMLDivElement | null>(
      null,
    );
    const [scrollbarX, setScrollbarX] = React.useState<
      ScrollAreaScrollbarElement | null
    >(null);
    const [scrollbarY, setScrollbarY] = React.useState<
      ScrollAreaScrollbarElement | null
    >(null);
    const [cornerWidth, setCornerWidth] = React.useState(0);
    const [cornerHeight, setCornerHeight] = React.useState(0);
    const [scrollbarXEnabled, setScrollbarXEnabled] = React.useState(
      false,
    );
    const [scrollbarYEnabled, setScrollbarYEnabled] = React.useState(
      false,
    );
...
interface ScrollAreaScrollbarVisibleProps extends
  Omit<
...
interface ScrollAreaScrollbarAxisProps extends
  Omit<
...
>((props: ScopedProps<ScrollAreaScrollbarAxisProps>, forwardedRef) => {
  ...
  return (
    <ScrollAreaScrollbarImpl
      ...
      style={{
        bottom: 0,
        left: context.dir === "rtl"
          ? "var(--radix-scroll-area-corner-width)"
          : 0,
        right: context.dir === "ltr"
          ? "var(--radix-scroll-area-corner-width)"
          : 0,
...
const [ScrollbarProvider, useScrollbarContext] = createScrollAreaContext<
  ScrollbarContext
>(SCROLLBAR_NAME);
...
interface ScrollAreaScrollbarImplProps
  extends PrimitiveDivProps, ScrollAreaScrollbarImplPrivateProps {}
...
function getThumbSize(sizes: Sizes) {
  ...
  const scrollbarPadding = sizes.scrollbar.paddingStart +
...
function getScrollPositionFromPointer(
  ...
  const maxPointerPos = sizes.scrollbar.size - sizes.scrollbar.paddingEnd -
    thumbOffsetFromEnd;
  const maxScrollPos = sizes.content - sizes.viewport;
