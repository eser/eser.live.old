...
import * as PopperPrimitive from "../popper/mod.ts";
import { createPopperScope } from "../popper/mod.ts";
...
import { VisuallyHidden } from "../visually-hidden/mod.ts";
...
const [SelectProvider, useSelectContext] = createSelectContext<
  SelectContextValue
>(SELECT_NAME);

type NativeOption = React.VNode<React.ComponentProps<"option">>;
...
const Select: React.FC<SelectProps> = (
  ...
  const [trigger, setTrigger] = React.useState<
    SelectTriggerElement | null
  >(
    null,
  );
  const [valueNode, setValueNode] = React.useState<
    SelectValueElement | null
  >(
    null,
  );
  const [valueNodeHasChildren, setValueNodeHasChildren] = React.useState(
    false,
  );
  ...
  (props: ScopedProps<SelectTriggerProps>, forwardedRef) => {
    ...
    const [searchRef, handleTypeaheadSearch, resetTypeahead] =
      useTypeaheadSearch((search) => {
        ...
        const currentItem = enabledItems.find((item) =>
      ...
      <PopperPrimitive.Anchor asChild {...popperScope}>
        <Primitive.button
          ...
          onKeyDown={composeEventHandlers(triggerProps.onKeyDown, (event) => {
            ...
            const isModifierKey = event.ctrlKey || event.altKey ||
              event.metaKey;
            if (
...
const [SelectContentProvider, useSelectContentContext] = createSelectContext<
...
>(
  (props: ScopedProps<SelectContentImplProps>, forwardedRef) => {
    ...
    const [content, setContent] = React.useState<
      SelectContentImplElement | null
    >(null);
    const [viewport, setViewport] = React.useState<
      SelectViewportElement | null
    >(null);
    const composedRefs = useComposedRefs(
      forwardedRef,
    ...
    const focusFirst = React.useCallback(
      (candidates: Array<HTMLElement | null>) => {
        const [firstItem, ...restItems] = getItems().map((item) =>
          item.ref.current
        );
    ...
    const itemRefCallback = React.useCallback(
      (node: SelectItemElement | null, value: string, disabled: boolean) => {
        ...
        const isSelectedItem = context.value !== undefined &&
          context.value === value;
    ...
    const handleItemLeave = React.useCallback(() => content?.focus(), [
      content,
    ]);
    ...
    const SelectPosition = position === "popper"
...
>((props: ScopedProps<SelectItemAlignedPositionProps>, forwardedRef) => {
  ...
  const [contentWrapper, setContentWrapper] = React.useState<
    HTMLDivElement | null
  >(null);
  const [content, setContent] = React.useState<
    SelectItemAlignedPositionElement | null
  >(null);
  const composedRefs = useComposedRefs(
    forwardedRef,
    (node) => setContent(node),
  ...
  const position = React.useCallback(() => {
    if (
      ...
      const fullContentHeight = contentBorderTopWidth + contentPaddingTop +
        itemsHeight + contentPaddingBottom + contentBorderBottomWidth; // prettier-ignore
      ...
      const topEdgeToTriggerMiddle = triggerRect.top + triggerRect.height / 2 -
        CONTENT_MARGIN;
      const triggerMiddleToBottomEdge = availableHeight -
        topEdgeToTriggerMiddle;

      const selectedItemHalfHeight = selectedItem.offsetHeight / 2;
      const itemOffsetMiddle = selectedItem.offsetTop + selectedItemHalfHeight;
      const contentTopToItemMiddle = contentBorderTopWidth + contentPaddingTop +
        itemOffsetMiddle;
      const itemMiddleToContentBottom = fullContentHeight -
      ...
      if (willAlignWithoutTopOverflow) {
        ...
        const viewportOffsetBottom = content.clientHeight - viewport.offsetTop -
        ...
        const height = contentTopToItemMiddle +
        ...
        const height = clampedTopEdgeToTriggerMiddle +
          itemMiddleToContentBottom;
        contentWrapper.style.height = height + "px";
        viewport.scrollTop = contentTopToItemMiddle - topEdgeToTriggerMiddle +
          viewport.offsetTop;
      }
...
const [SelectGroupContextProvider, useSelectGroupContext] = createSelectContext<
...
const [SelectItemContextProvider, useSelectItemContext] = createSelectContext<
...
>(
  (props: ScopedProps<SelectItemTextProps>, forwardedRef) => {
    ...
    const [itemTextNode, setItemTextNode] = React.useState<
      SelectItemTextElement | null
    >(null);
    const composedRefs = useComposedRefs(
      forwardedRef,
...
>((props: ScopedProps<SelectScrollButtonImplProps>, forwardedRef) => {
  ...
  useLayoutEffect(() => {
    const activeItem = getItems().find((item) =>
      item.ref.current === document.activeElement
    );
...
const BubbleSelect = React.forwardRef<
  HTMLSelectElement,
  Radix.ComponentPropsWithoutRef<"select">
>(
  (props, forwardedRef) => {
