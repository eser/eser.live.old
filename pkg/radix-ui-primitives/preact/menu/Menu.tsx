...
import { composeRefs, useComposedRefs } from "../compose-refs/mod.ts";
...
const [MenuProvider, useMenuContext] = createMenuContext<MenuContextValue>(
  MENU_NAME,
);
...
const [MenuRootProvider, useMenuRootContext] = createMenuContext<
...
const Menu: React.FC<MenuProps> = (props: ScopedProps<MenuProps>) => {
  ...
  const [content, setContent] = React.useState<MenuContentElement | null>(
    null,
  );
...
const [PortalProvider, usePortalContext] = createMenuContext<
  PortalContextValue
>(PORTAL_NAME, {
...
const [MenuContentProvider, useMenuContentContext] = createMenuContext<
...
type MenuContentImplPrivateProps = {
  ...
  disableOutsidePointerEvents?:
    DismissableLayerProps["disableOutsidePointerEvents"];
                ...
                <PopperPrimitive.Content
                  ...
                  onKeyDown={composeEventHandlers(
                    ...
                    (event) => {
                      ...
                      event.preventDefault();
                      const items = getItems().filter((item) => !item.disabled);
                      const candidateNodes = items.map((item) =>
                        item.ref.current!
                      );
...
const [MenuSubProvider, useMenuSubContext] = createMenuContext<
  MenuSubContextValue
>(SUB_NAME);
...
interface MenuSubContentProps extends
  Omit<
    MenuContentImplProps,
