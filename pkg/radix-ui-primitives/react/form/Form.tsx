...
const Form = React.forwardRef<FormElement, FormProps>(
  (props: ScopedProps<FormProps>, forwardedRef) => {
    ...
    const handleFieldCustomErrorsChange:
      ValidationContextValue["onFieldCustomErrorsChange"] = React.useCallback(
    ...
    const handleFieldMessageIdAdd:
      AriaDescriptionContextValue["onFieldMessageIdAdd"] = React.useCallback(
        (fieldName, id) => {
    ...
    const handleFieldMessageIdRemove:
      AriaDescriptionContextValue["onFieldMessageIdRemove"] = React.useCallback(
        (fieldName, id) => {
          setMessageIdsMap((prevMessageIdsMap) => {
            const fieldDescriptionIds = new Set(prevMessageIdsMap[fieldName]);
    ...
    const getFieldDescription:
      AriaDescriptionContextValue["getFieldDescription"] = React.useCallback(
        (fieldName) =>
...
const FormControl = React.forwardRef<FormControlElement, FormControlProps>(
  (props: ScopedProps<FormControlProps>, forwardedRef) => {
    ...
    const customMatcherEntries = validationContext.getFieldCustomMatcherEntries(
      name,
    );
    ...
    const updateControlValidity = React.useCallback(
      async (control: FormControlElement) => {
        ...
        const hasSyncCustomErrors = Object.values(syncCustomErrorsById).some(
          Boolean,
        );
        ...
        if (!hasSyncCustomErrors && ayncCustomMatcherEntries.length > 0) {
          ...
          const hasAsyncCustomErrors = Object.values(asyncCustomErrorsById)
            .some(Boolean);
...
>(
  (props: ScopedProps<FormBuiltInMessageProps>, forwardedRef) => {
    const { match, forceMatch = false, name, children, ...messageProps } =
      props;
...
>(
  (props: ScopedProps<FormCustomMessageProps>, forwardedRef) => {
    ...
    const customMatcherEntry = React.useMemo(() => ({ id, match }), [
      id,
      match,
    ]);
...
function getFirstInvalidControl(
  ...
  const [firstInvalidControl] = Array.from(elements).filter(isHTMLElement)
    .filter(isInvalid);
...
function isAsyncCustomMatcherEntry(
  ...
  return entry.match.constructor.name === "AsyncFunction" ||
